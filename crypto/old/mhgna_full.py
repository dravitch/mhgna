...
def backtest_strategy(data, returns, volatility, momentum_scores):
    ...
    # Utiliser uniquement les dates présentes dans l'index de data pour éviter les problèmes
    ...
    results = pd.DataFrame(index=valid_dates[:len(portfolio_value)-1],
                          columns=['Portfolio Value', 'Benchmark Value', 'Drawdown'])
    
    # Assigner les valeurs (en s'assurant que les longueurs correspondent)
    min_length = min(len(results), len(portfolio_value)-1, len(benchmark_value)-1, len(drawdown_history))
